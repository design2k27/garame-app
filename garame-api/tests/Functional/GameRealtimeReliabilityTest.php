...
class GameRealtimeReliabilityTest extends WebTestCase
{
    ...
    public function testPollReturnsChangedFalseWhenClientIsInSync(): void
    {
    ...
    public function testPollReturnsChangedTrueWhenClientIsOutOfSync(): void
    {
        [$alice, , $game] = $this->createPlayingGame();
        ...
        $this->authenticateClient($alice);
        $this->client->request('GET', sprintf('/api/games/%s/poll?sinceVersion=1', $game->getId()));
    ...
    public function testAckStoresClientVersion(): void
    {
        [$alice, , $game] = $this->createPlayingGame();
        $this->em->flush();
        ...
        $this->assertSame($game->getStateVersion(), $payload['ackedStateVersion']);
        $this->assertSame($game->getStateVersion(), $payload['serverVersion']);
        $this->assertTrue($payload['inSync']);
    }

    public function testAckRejectsVersionAboveServerVersion(): void
    {
        [$alice, , $game] = $this->createPlayingGame();
        $this->em->flush();
        ...
        $this->client->request(
            ...
            sprintf('/api/games/%s/ack', $game->getId()),
            server: ['CONTENT_TYPE' => 'application/json'],
            content: json_encode(['stateVersion' => $game->getStateVersion() + 1], JSON_THROW_ON_ERROR)
        );
        ...
        $this->assertSame($game->getStateVersion(), $payload['error']['details']['serverVersion']);
    }

    public function testPlayRejectsOutOfSyncClientVersion(): void
    {
        [$alice, , $game] = $this->createPlayingGame();
        $game->bumpStateVersion();
        $this->em->flush();
